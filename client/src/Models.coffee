...
class StepsView extends Backbone.View
    ...
    isIngrediant: (o) ->
        return not o.reqs?
    render: ->
        output = "<hr style='border-top:1px solid #666;'><h2>Steps</h2>"
        step_data = @model.get "steps"
        output += "<table class='table table-striped'>"
        output += "<tr><th>#</th><th>Name</th><th>Description</th></tr>"
        for s in step_data
            output += "<tr>"
            if @isIngrediant s
                output += "<th>#{s.id}</th><td>#{s.name}</td><td>#{s.quant.a} #{s.quant.u} of #{s.data.Long_Desc}</td>"
            else
                output += "<th>#{s.id}</th><td>#{s.name}</td><td>#{s.op} steps #{s.reqs} for #{s.time.a} #{s.time.u}s"
